...
export function QuoteRequestCard({ order }: QuoteRequestCardProps) {
  ...
  return (
    <div
      className={`bg-white border rounded-xl shadow-sm overflow-hidden transition-all hover:shadow-md ${!isViewed ? "ring-2 ring-blue-400" : ""}`}
    >
      {/* Badge "NOVO" */}
      {!isViewed && (
      ...
      >
        <div className="flex items-start gap-4">
          <div
            className={`w-3 h-3 mt-2 rounded-full ${!isViewed ? "bg-blue-500 animate-pulse" : "bg-gray-300"}`}
          />
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden md:block">
            <span className="text-sm font-medium">
              {order.total_items} ite{order.total_items > 1 ? "ns" : "m"}
            </span>
          </div>
          <svg
